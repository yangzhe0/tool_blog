...
- 原子时及天体动力学时：以全球数百台原子钟为基础，定义了极为均匀的时间（例如 TAI、UTC、TT、TDB、TCB）。
- 地球自转时：以地球自转为基础，反映地球实际自转状态（如 UT1、UT0、UT2）；由于地球自转存在周期性变化及长期漂移，因此需要通过闰秒调整使民用时间（UTC）与其保持接近。

此外，还存在恒星时，即以背景恒星为参考的时间，用于校正天球坐标和天文观测，暂不做讨论。

下表对历史上的原子时、力学时、地球时做了系统的归纳。
...
| 序号 | TS                | NUMBER | 记录格式           | 完成情况 | 待处理项     | 观测年代 |
| ---- | ----------------- | ------ | ------------------ | -------- | ------------ | -------- |
| 1    | ET                | 419    | Day_detail         | 完成     |              |          |
| 2    | TDB               | 54     | JD                 |          |              |          |
| 3    | GMT               | 562    | Full               |          |              |          |
| 4    | TCB               | 6302   | Epoch_TCB          |          |              |          |
| 5    | TT                | 966    | JD,MJD,Full        |          | jg0053       |          |
| 6    | UT1               | 808    | Full               |          |              |          |
...
1972年之后的记录，直接使用SOFA程序内嵌的闰秒表进行转换。

1972年之前的记录，统一使用10s作为ΔAT与TAI对齐。

#### 2.2.4 UT1-TT

UT1的整体思路是通过已有的ΔT数据加上UT1的时间转换到TT
1972年之后的记录，使用IERS发布的UT1-UTC差值（finals2000A.all.csv），经由iauUt1utc以及iauUtctt函数转换到TT。
1956至1973的记录，使用IERS发布的UT1-TAI差值（EOP_C01_IAU2000），经由iauUt1tai以及iauTaitt函数转换到TT。
1846年之前的记录，使用文献*Long-term changes in the rotation of the Earth: 700 B.G. to A.D. 1980*中记录的ΔT数据，并使用三次样条插值，插值到月以提高数据精度。另外的，NASA的ΔT长期多项式模型（Polynomial Approximation for TT-UT1）也可以外推估计ΔT。
